<script lang="ts">
    ...
    import { NAV_SEARCH_LIMIT, type NavResult, type NavSymbol } from '$lib/nav';
    ...
    // --- lazily loaded file symbols ------------------------------------------

    /** file name -> symbols, filled in as files are opened. */
    let loaded = $state<Record<string, NavSymbol[]>>({});
    let open = $state<Record<string, boolean>>({});
    let loading = $state<Record<string, boolean>>({});

    // The file holding the current symbol arrives with the page, so landing on a
    // type shows its neighbours immediately.
    $effect(() => {
        const file = data.openFile;
        if (!file) return;
        untrack(() => {
            loaded[file] ??= data.openSymbols;
            open[file] ??= true;
        });
    });

    const navUrl = () =>
        resolve('/s/[...module=module]/[version=version]/nav', {
            module: data.module,
    ...
    async function toggleFile(file: string) {
        open[file] = !open[file];
        if (!open[file] || loaded[file] || loading[file]) return;

        loading[file] = true;
        try {
            const res = await fetch(`${navUrl()}?file=${encodeURIComponent(file)}`);
            loaded[file] = ((await res.json()) as NavResult).symbols;
        } finally {
            loading[file] = false;
        }
    }
    ...
    let query = $state('');
    let results = $state<NavResult | undefined>();
    ...
    function onQuery(value: string) {
        ...
        timer = setTimeout(async () => {
            ...
            inflight = controller;
            try {
                const res = await fetch(`${navUrl()}?q=${encodeURIComponent(value)}`, {
                    signal: controller.signal
                });
                results = (await res.json()) as NavResult;
            } catch {
    ...
    const symbolHref = (fqn: string) =>
        resolve('/s/[...module=module]/[version=version]/t/[fqn]', {
            module: data.module,
            version: data.version,
            fqn
        });
...
{#snippet symbolLink(sym: NavSymbol, indent: number)}
    <a
        href={symbolHref(sym.fqn)}
        title={sym.fqn}
        class="flex items-center gap-2 rounded px-2 py-1 font-mono text-[13px] hover:bg-zinc-100 dark:hover:bg-zinc-900 {page
            ...
            : ''}"
        style="padding-left: {indent + sym.depth * 0.75}rem"
    >
        <span class="w-3 shrink-0 text-[10px] text-zinc-400">{kindMark[sym.kind]}</span>
        <span class="truncate {sym.deprecated ? 'line-through' : ''}">{sym.name}</span>
    </a>
{/snippet}
...
<div class="min-h-dvh md:flex">
    ...
    >
        ...
        <span class="truncate font-mono text-xs text-zinc-500">{data.module}:{data.version}</span>
        <a
    ...
    >
        ...
        <nav class="flex-1 overflow-y-auto px-2 py-3 text-sm">
            {#if results}
                <p class="px-2 pb-1 text-[11px] text-zinc-400">
                    {#if results.total > results.symbols.length}
                        showing {results.symbols.length} of {results.total}
                        {results.total > NAV_SEARCH_LIMIT ? '· narrow the search' : ''}
                    {:else}
                        {results.total}
                        {results.total === 1 ? 'match' : 'matches'}
                    {/if}
                </p>
                {#each results.symbols as sym (sym.fqn)}
                    {@render symbolLink(sym, 0.5)}
                {:else}
                    <p class="px-2 text-xs text-zinc-500">No symbols match.</p>
                {/each}
            {:else if searching}
                <p class="px-2 text-xs text-zinc-500">Searching…</p>
            {:else}
                {#each data.packages as pkg (pkg.name)}
                    <div class="mb-3">
                        <p class="px-2 pb-0.5 font-mono text-[11px] break-all text-zinc-400">
                            {pkg.name}
                        </p>
                        {#each pkg.files as file (file.name)}
                            <button
                                ...
                                aria-expanded={open[file.name] === true}
                                title={file.name}
                                class="flex w-full items-baseline gap-1.5 rounded px-2 py-0.5 text-left font-mono text-[11px] text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900"
                            >
                                <span class="w-2 shrink-0">{open[file.name] ? '−' : '+'}</span>
                                <span class="truncate">{file.name.split('/').pop()}</span>
                                <span class="ml-auto shrink-0 text-zinc-400">{file.symbols}</span>
                            </button>

                            {#if open[file.name]}
                                {#if loading[file.name] && !loaded[file.name]}
                                    <p class="px-2 py-1 pl-6 text-[11px] text-zinc-400">loading…</p>
                                {:else}
                                    {#each loaded[file.name] ?? [] as sym (sym.fqn)}
                                        {@render symbolLink(sym, 1.25)}
                                    {/each}
                                {/if}
                            {/if}
                        {/each}
